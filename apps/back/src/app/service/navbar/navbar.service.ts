...
import { JSONldListNavbar, Navbar } from "../../model/navbar";
...
export class NavbarService {
    ...
    getNavbars() {
        return this.httpClient
            .get<JSONldListNavbar>(this.baseUrl + this.normalUrl, {
    ...
    getNavbarsByPage(pageValue: string) {
        return this.httpClient
            .get<JSONldListNavbar>(this.baseUrl + pageValue, {
                headers: this.LDJsonHeader,
            })
            .pipe(
                map((res) => ({
    ...
    getThisNavbar(value: number) {
        return this.httpClient
            .get<Navbar>(this.baseUrl + this.getUrl + value, {
    ...
    toggleNavbarStatus(id: number) {
        ...
        this.httpClient
            .patch<Navbar>(this.baseUrl + this.updateStatusUrl + id, {
                headers: this.MergeJsonHeader,
            })
            .subscribe({
                next: (value) => {
                    ...
                    if (value.status === true) {
                        this.alert.success(
                            "Navbar succesfully enabled",
                            options
                        );
                    } else {
                        this.alert.success(
                            "Navbar succesfully disabled",
                            options
                        );
                    }
                ...
                error: (value) => {
                    ...
                    this.alert.error(
                        "A navbar is already active disable it before",
                        options
                    );
